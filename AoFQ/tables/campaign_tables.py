...
primary_objectives_desc=[
    "The sentries must have at least one villain, which gets Tou +3 and Qua +1, and counts as the AI goal. The objective is completed when the villain is killed",
    "Whenever all enemies are killed, a wave of reinforcements is deployed at the end of the round. The centre of the table is the AI goal. The objective is completed after all sentires and the first wave of reinforcements has been killed.",
    "Whenever an enemy unit is destroyed, roll one die, on a 6 it drops an objective marker, which counts as the AI goal. Heroes may pick the marker up by using a skill action whilst within 1\", and drop it on the spot if they take any damage. The objective is completed once a hero carries the marker within 3\" of the furthest table corner from where the marker was dropped.",
...
secondary_objectives_desc=[
    'One random enemy unit from the reinforcements becomes the AI goal. When that unit is destroyed, replace it with a marker. Heroes within 1" may take a skill action, and if they pass a Wil test the objective is completed.',
    ...
    'Place one objective marker within 6" of the reinforcement table corner, which becomes the AI goal. Heroes may use a skill action whilst within 1“ of a marker, and if they pass a Dex test the marker is removed and the objective is completed.',
    'Place one objective marker at the centre of the table, which becomes the AI goal. At the end of each round, if a hero is within 3" of the marker whilst enemies aren\'t, one hero within 3" of it may take a Dex test, and add a token to it if passed. The objective is completed once the marker has 2 tokens.',
